...
export const BatchSchedule: React.FC<{ data: BatchScheduleContentSection }> = ({
  ...
  // Dynamic schedule items from Strapi data
  const scheduleItems = [
    {
      ...
      <div className="container mx-auto max-w-screen-xl">
        ...
        <div className="text-center space-y-4">
          ...
          <Button size="lg" className="rounded-full text-base ">
            {data.nextBatchScheduleBtn?.btnIcon && (
              <DynamicIcon
                icon={data.nextBatchScheduleBtn.btnIcon}
                className="w-4 h-4 mr-2"
                width={16}
                height={16}
              />
            )}
            {data.nextBatchScheduleBtn?.btnLabel || 'Register Now'}
          </Button>
        </div>
      </div>
    </section>
